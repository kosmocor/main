...
- 시나리오는 readiness 설정상태에서 부하를 준 후, readiness를 제거할 것이다.

```
kubectl apply -f kubectl apply -f deployment_all.yml
```

- seige 로 부하를 주면서 워크로드를 모니터링 함.
```
...
배포기간중 Availability 가 평소 100%에서 85% 대로 떨어지는 것을 확인. 원인은 쿠버네티스가 성급하게 새로 올려진 서비스를 READY 상태로 인식하여 서비스 유입을 진행한 것이기 때문. 이를 막기위해 Readiness Probe 를 설정하였다.
